...
import {
  getAdminCourse, getCreatorProfile,
...
export default function CourseEditor() {
  ...
  useEffect(() => {
    // Load school slug for blob storage organisation
    getCreatorProfile().then(r => { if (r.success) setSchoolSlug((r.data as any)?.slug ?? ''); });
  ...
  const handleSaveCourse = async () => {
    ...
    if (result.success) {
      ...
      if (isNew) router.push('/creator/admin/courses');
    } else {
  ...
  const handleDeleteCourse = async () => {
    ...
    if (result.success) router.push('/creator/admin/courses');
  ...
  if (loadError) return (
    <div className="max-w-md mx-auto px-4 py-16 text-center">
      ...
      <Link href="/creator/admin/courses" className="inline-flex items-center gap-2 px-5 py-2.5 bg-primary text-primary-foreground text-sm font-semibold rounded-xl hover:bg-primary/90 transition-colors">
        ← Back to Courses
      </Link>
    </div>
  );
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      {/* Back link */}
      <Link href="/creator/admin/courses" className="inline-flex items-center gap-1.5 text-sm text-muted-foreground hover:text-foreground mb-6 transition-colors">
