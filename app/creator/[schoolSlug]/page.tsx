      ...
      <header style={{ position: 'sticky', top: 0, zIndex: 50, background: 'rgba(255,255,255,0.88)', backdropFilter: 'blur(16px)', borderBottom: `1px solid ${C.border}` }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 24px', height: 64, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16 }}>
          ...
          <nav style={{ display: 'flex', gap: 4 }}>
            <Link href='/learn/creators' style={{ padding: '6px 13px', fontSize: 13, fontWeight: 600, color: C.muted, borderRadius: 8, textDecoration: 'none' }}>← Creators</Link>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
            {session?.user ? (
              <Link href='/learn/dashboard' style={{ padding: '10px 20px', background: C.green, color: '#fff', borderRadius: 10, fontSize: 13, fontWeight: 700, textDecoration: 'none', boxShadow: `0 4px 14px rgba(14,159,110,.3)` }}>
                My Learning →
              </Link>
            ) : (
              <>
