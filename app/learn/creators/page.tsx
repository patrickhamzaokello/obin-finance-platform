...
export default async function BrowseCreators({ searchParams }: Props) {
  ...
  // Creators with their published course count and total enrolled learners
  ...
  const filtered = creators.filter((c) => {
    ...
    // Only show creators with at least one published course
    ...
    <div className="space-y-8">
      ...
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          {filtered.length} creator{filtered.length !== 1 ? 's' : ''} on Pkasemer
        </p>
      </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {filtered.map((creator) => {
            ...
            return (
              ...
              >
                {/* Banner */}
                <div className="h-28 bg-gradient-to-br from-primary/20 to-primary/5 overflow-hidden relative">
                  {banner ? (
                    <img
                      src={banner}
                      alt=""
