...
import { ArrowLeft, Users, BookOpen, ExternalLink, Percent } from 'lucide-react';
// ExternalLink kept for the "View profile" button
...
export default async function PlatformSchoolDetailPage({
  ...
  return (
    <div className='px-8 py-8 max-w-4xl'>
      <Link href="/admin/schools" className='inline-flex items-center gap-2 text-sm text-muted-foreground hover:text-primary mb-6 transition-colors'>
      ...
      <div className='flex items-start justify-between mb-8'>
        <div>
          ...
          <p className='text-sm text-muted-foreground font-mono mt-1'>/creator/{s.slug}</p>
        </div>
      ...
      <div className='grid grid-cols-3 gap-4 mb-8'>
        ...
        <div className='bg-white rounded-2xl shadow-sm p-5'>
          <div className='flex items-center gap-2 text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-2'>
            <Percent size={13} /> Platform Commission
          </div>
          <CommissionEditor schoolId={s.id} current={s.commissionPercent ?? 10} />
        </div>
      </div>
      ...
      <div className='bg-white rounded-2xl shadow-sm overflow-hidden'>
        ...
        {courses.length === 0 ? (
          <p className='px-6 py-8 text-sm text-muted-foreground text-center'>No classes yet.</p>
        ) : (
