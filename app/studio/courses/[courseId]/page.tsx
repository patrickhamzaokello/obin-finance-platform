...
export default function CourseEditor() {
  ...
  const handleSaveCourse = async () => {
    ...
    if (result.success) {
      ...
      if (isNew) router.push('/studio/courses');
    } else {
  ...
  const handleDeleteCourse = async () => {
    ...
    if (result.success) router.push('/studio/courses');
  ...
  if (loadError) return (
    <div className="max-w-md mx-auto px-4 py-16 text-center">
      ...
      <Link href="/studio/courses" className="inline-flex items-center gap-2 px-5 py-2.5 bg-primary text-primary-foreground text-sm font-semibold rounded-xl hover:bg-primary/90 transition-colors">
        ← Back to Courses
      </Link>
    </div>
  );
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      {/* Back link */}
      <Link href="/studio/courses" className="inline-flex items-center gap-1.5 text-sm text-muted-foreground hover:text-foreground mb-6 transition-colors">
