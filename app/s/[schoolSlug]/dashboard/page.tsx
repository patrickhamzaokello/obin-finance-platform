        ...
        <div className='max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-5 flex justify-between items-center'>
          <div className='flex items-center gap-4'>
            ...
            <div>
              ...
              <p className='text-xs text-muted-foreground mt-0.5'>Welcome back, {session.user.name || session.user.email}</p>
            </div>
          </div>
          <div className='flex items-center gap-3'>
            {isAdmin && (
              <Link href='/admin'
                className='inline-flex items-center gap-1.5 px-3 py-2 text-sm font-semibold text-primary border border-primary/30 rounded hover:bg-primary/5 transition-colors'>
            ...
            <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5'>
              {enrolledCourses.map((c: any) => (
                <Link href={`/learning/${c.id}`} key={c.id}>
                  <div className='group bg-white border border-border border-l-[3px] border-l-primary rounded overflow-hidden hover:shadow-md transition-all duration-200 h-full flex flex-col cursor-pointer'>
                ...
                <Link href={`/course/${c.id}`} key={c.id}>
                  <div className='group bg-white border border-border rounded overflow-hidden hover:border-primary hover:shadow-md transition-all duration-200 h-full flex flex-col cursor-pointer'>
                    ...
                    <div className='p-4 flex-1 flex flex-col'>
                      <h3 className='font-semibold text-foreground leading-snug'>{c.title}</h3>
                      <p className='text-muted-foreground text-sm mt-2 line-clamp-2 flex-1'>{c.description}</p>
                      {c.instructor && <p className='text-xs text-muted-foreground mt-3'>By {c.instructor}</p>}
                      <div className='mt-4 flex items-center justify-between text-sm'>
