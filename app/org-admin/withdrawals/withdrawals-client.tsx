...
const STATUS = {
  ...
  org_approved:  { label: 'Approved',     color: 'bg-blue-50 text-blue-700',    icon: CheckCircle2 },
  owner_approved:{ label: 'Awaiting pay', color: 'bg-indigo-50 text-indigo-700', icon: CheckCircle2 },
...
export function OrgWithdrawalsClient({
  rows,
  onApprove,
  onReject,
}: {
  ...
  async function doApprove(id: string) {
    ...
    if (res.success) {
      setLocalRows(r => r.map(w => w.id === id ? { ...w, status: 'org_approved', orgApprovedAt: new Date() } : w));
    } else {
  ...
  const pending = localRows.filter(w => w.status === 'pending');
  const rest    = localRows.filter(w => w.status !== 'pending');
    ...
    <div className='max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6'>
      ...
      <div>
        ...
        <p className='text-sm text-muted-foreground mt-1'>
          Review and approve withdrawal requests from creators in your organisation.
        </p>
      </div>
      ...
      <div>
        <h2 className='text-sm font-semibold text-foreground mb-3 flex items-center gap-2'>
          <span className='inline-flex items-center justify-center w-5 h-5 rounded-full bg-amber-100 text-amber-700 text-xs font-bold'>{pending.length}</span>
            ...
            {pending.map(w => (
              <div key={w.id} className='bg-white rounded-2xl shadow-sm p-5 flex items-center gap-4 flex-wrap'>
                ...
                <div className='flex-1 min-w-0'>
                  ...
                  <p className='text-xs text-muted-foreground/60'>{fmtDate(w.requestedAt)}</p>
            ...
            {rest.map(w => {
              ...
              return (
                <div key={w.id} className='px-6 py-4 flex items-center gap-4'>
                  ...
                  <p className='text-xs text-muted-foreground shrink-0'>{fmtDate(w.requestedAt)}</p>
                </div>
              );
            })}
          </div>
        ...
        <div className='fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4'>
          <div className='bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 space-y-4'>
            ...
            <textarea
              ...
              placeholder='e.g. Please verify your phone number'
